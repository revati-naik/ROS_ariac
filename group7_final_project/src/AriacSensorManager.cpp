        ...
        parts_from_bin_unreachable {}
{
    ...
    sensor_balckout_sub = sensor_nh_.subscribe("/ariac/lc_bin_6", 10,
                                           &AriacSensorManager::SensorBackOutcallback, this);
...
void AriacSensorManager::SensorBackOutcallback(const osrf_gear::LogicalCameraImage::ConstPtr & image_msg){
...
//        ROS_WARN_STREAM("[bb_coming_callback] : Not sensor blackOut" );
    ...
    if (value >7){
        ROS_ERROR_STREAM_THROTTLE(10,"[bb_coming_callback] : It is a blackOut" );
...
//        quality_flag =  qc_1_redFlag;
//
//        std::string tray_ID = "world";
//        std::string camera_ID = "quality_control_sensor_" + std::to_string(agv_id) + "_frame";
//
//        ROS_INFO_STREAM("[AriacSensorManager][QualityCheck]: quality_image1.models.size() is: "<< quality_image1->models.size());
//
//        for (auto& msg : quality_image1->models) {
//            auto part_type = msg.type;
//            auto part_pose = msg.pose;
//
//            ROS_INFO_STREAM("[AriacSensorManager][QualityCheck]: part_type in qc1 Frame: "<< part_type);
//            ROS_INFO_STREAM("[AriacSensorManager][QualityCheck]: part_pose_kit in qc1 Frame: "<< part_pose);
//
//            geometry_msgs::PoseStamped StampedPose_in, StampedPose_out;
...
//            geometry_msgs::Pose part_pose_kit = StampedPose_out.pose;
//
//            ROS_INFO_STREAM("[AriacSensorManager][QualityCheck]: part_type in world Frame: "<< part_type);
//            ROS_INFO_STREAM("[AriacSensorManager][QualityCheck]: part_pose_kit in world Frame: "<< part_pose_kit);
//
//        }
//    }
//    else if (agv_id == 2){
...
void AriacSensorManager::CorrectPose(geometry_msgs::Pose current_pose, geometry_msgs::Pose updated_pose,
    ...
    if(agv_id == 1) { // straight, basically to remove from their original locations
        this_arm = &arm;
        that_arm = &arm2;
//        Other_tray_ID = 2;
//        This_tray_ID = 1;
    ...
    else if(agv_id == 2){ // reverse, basically to add back to their original locations
        this_arm = &arm2;
        that_arm = &arm;
//        Other_tray_ID = 1;
//        This_tray_ID = 2;
        ...
        for(const auto &shipment: shipments){
            ...
            if(agv_id == belt_tray_id)
            {
                ...
                for (const auto &product: products)
                {
                    ...
                    {
                        ROS_INFO_STREAM("Already inside the parts from belt for loop");
                        product_type_pose_.first = product.type;
...
//                 while (!parts_from_belt_temp.empty())
//                 {
//                     // wait for that arm if actually collects correct type and number of belt parts
//                     ROS_INFO_STREAM_THROTTLE(10, "[ExecuteOrder]: wait for that arm to collect enought numbers of belt parts...");
//                     if (sensor_black_out){
//                         ROS_WARN_STREAM("[ExecuteOrder]: blackout detected ");
//                         break;
//                     }
//                 }
//
//                 if (!parts_from_belt_temp.empty()) {
//                     if (sensor_black_out)
//                         ROS_WARN_STREAM("[ExecuteOrder]: Contiue building kit because of sensor black out and part numbers on the tray matches needed number ... ");
//                 }
//                 else {
//                     ROS_WARN_STREAM("[ExecuteOrder]: That arm collects enough of correct belt parts");
//                 }
//                 // commment the above lines
